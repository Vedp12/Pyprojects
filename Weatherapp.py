...
def generate_pdf(data: dict, headlines: list[str]) -> str:
    ...
    story.append(
        Paragraph(
            "🌤  Weather Report",
            _style(
                "Title", fontSize=26, textColor=colors.HexColor("#1a73e8"), spaceAfter=4
            ),
        )
    )
    story.append(
    ...
    story.append(
        HRFlowable(
            width=W, thickness=1.5, color=colors.HexColor("#1a73e8"), spaceAfter=16
        )
    )
    ...
    table_data = [
        ...
        [
            "Visibility",
            f"{data['visibility_m']} m" if data["visibility_m"] != "N/A" else "N/A",
        ],
    ...
    story.append(
        HRFlowable(
            width=W, thickness=1, color=colors.HexColor("#dadce0"), spaceAfter=12
        )
    )
    story.append(
        Paragraph(
    ...
    story.append(
        HRFlowable(
            width=W, thickness=0.5, color=colors.HexColor("#dadce0"), spaceAfter=8
        )
    )
    story.append(
        Paragraph(
...
def print_weather(data: dict) -> None:
    ...
    print(f"  {bar}")
    print(
        f"  🌡  Temperature : {data['temperature_c']} °C  (feels {data['feels_like_c']} °C)"
    )
